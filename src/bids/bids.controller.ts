...
import { CloneRepriceBidDto, CloneRepriceBidResponseDto } from './dto/clone-reprice-bid.dto';
import { CalculateBidDto, CalculateBidResponseDto } from './dto/calculate-bid.dto';
import {
  ApiTags,
  ApiBearerAuth,
  ApiOperation,
...
export class BidsController {
  ...
  @ApiOperation({
    ...
    description:
      'Доступно для: Администратор, ПТО, Менеджер. Создает заявку с автоматической генерацией номера.',
  })
  @ApiBody({
    type: CreateBidDto,
  ...
  @ApiOperation({
    ...
    description:
      'Возвращает актуальные расчеты для текущей конфигурации без сохранения заявки в БД',
  })
  @ApiBody({ type: CalculateBidDto })
  @ApiResponse({
    status: 201,
    description: 'Черновик заявки успешно пересчитан',
    type: CalculateBidResponseDto,
  })
  calculateDraft(@Body() dto: CalculateBidDto) {
  ...
  @ApiBody({ type: CloneRepriceBidDto })
  @ApiResponse({
    status: 201,
    description: 'Новая заявка после пересчета успешно создана',
    type: CloneRepriceBidResponseDto,
  })
  cloneReprice(@Param('id', ParseIntPipe) id: number, @Body() dto: CloneRepriceBidDto) {
    if (process.env.ENABLE_BID_CLONE_REPRICE === 'false') {
      throw new NotFoundException('Маршрут clone-reprice временно отключен');
    }
