...
export class CurrencySettingsService {
  ...
  private async recalculateMaterialsPriceInKzt(
    settings: CurrencySettings,
    ...
    this.logger.log(
      `Recalculate materials in KZT: affected currencies [${normalizedCurrencies.join(', ')}]`
    );
    ...
    for (let i = 0; i < updatedMaterials.length; i += batchSize) {
      ...
      this.logger.log(
        `Recalculate materials in KZT: saved ${Math.min(i + batch.length, updatedMaterials.length)}/${updatedMaterials.length}`
      );
    }
    this.logger.log(
      `Recalculate materials in KZT: done (${updatedMaterials.length} materials, ${changedPricesByMaterialId.size} changed)`
    );
  ...
  private async syncCalculationsWithCurrentMaterialPrices(
    pricesByMaterialId: Map<number, number>
  ): Promise<void> {
    ...
    calculations.forEach((calculation) => {
      const updatedData = this.updateCalculationDataByMaterials(
        calculation.data,
        pricesByMaterialId
      );
    ...
    this.logger.log(
      `Sync calculations with material prices: done (${updatedCalculations.length}/${calculations.length} updated)`
    );
  }
  ...
  async updateSettings(updateData: UpdateCurrencySettingsDto, changedBy?: string) {
    ...
    const rateFields: Array<keyof UpdateCurrencySettingsDto> = [
      'usdRate',
      'eurRate',
      'rubRate',
      'kztRate',
    ...
    const savedSettings =
      changedEntries.length > 0 ? await this.currencySettingsRepo.save(settings) : settings;
    this.logger.log(
      `Update currency settings: saved. Rate changes: ${hasRateChanges ? 'yes' : 'no'}`
    );

    if (hasRateChanges) {
      const changedPricesByMaterialId = await this.recalculateMaterialsPriceInKzt(
        savedSettings,
        affectedCurrencies
      );
    ...
    this.logger.log(
      `Update currency settings: done in ${elapsedMs}ms (${changedEntries.length} changed fields)`
    );
